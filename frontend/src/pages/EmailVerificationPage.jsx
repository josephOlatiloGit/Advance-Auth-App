...
export default function EmailVerificationPage() {
  ...
  const handleChange = (index, value) => {
    ...
    if (value.length > 1) {
      ...
      setCode(newCode);
      // focus on the last none-empty input or the first empty one
  ...
  return (
    <div className="max-w-md w-full bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden ">
      <motion.div
        ...
        className="ng-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-2xl p-8 w-full max-w-md"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex justify-between">
            {/* Here we map through the state code, and for each of the element in the array we return an input with attributes. Then we a ref the is matching with the current index so to bing the state with the input ref*/}
